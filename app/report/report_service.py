...
class ReportService:
    ...
    def generate_pdf_report(request: ReportRequest) -> io.BytesIO:
        ...
        if res_sl:
            pdf.cell(
                0, 8, f"Valor de Prioridad Total: {res_sl.get('max_value')}", ln=True
            )
            pdf.cell(
                0, 8, f"RAM Total Utilizada: {res_sl.get('total_weight')} GB", ln=True
            )
            pdf.ln(2)
            ...
            pdf.cell(0, 8, "Cálculo no realizado o datos pendientes.", ln=True)
        pdf.ln(5)
        ...
        if res_nr:
            ...
            pdf.cell(
                0, 8, f"Ruta Crítica: {' -> '.join(res_nr.get('path', []))}", ln=True
            )
        else:
            pdf.set_font("Helvetica", "I", 11)
            pdf.cell(0, 8, "Cálculo no realizado o datos pendientes.", ln=True)
